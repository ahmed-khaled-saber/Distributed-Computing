...
## on Asuumption that All Columns are Numeric ?
## otherwise, 
# numeric_cols <- sapply(heart_disease, is.numeric)
# heart_disease_numeric <- heart_disease[, numeric_cols]
# heart_disease_scaled <- as.data.frame(scale(heart_disease_numeric))
# heart_disease_scaled <- cbind(heart_disease[, !numeric_cols], heart_disease_scaled)
...
first_clust = kmeans(x=scaled, centers=k, nstart=1)
...
k <- 5 
second_clust = kmeans(x=scaled, centers=k, nstart=1)
...
clust_summary <- ....(data.frame, aggregate(. ~ hc_clust, data = ...., function(x) c(avg = ...., sd = ....)))
clust_summary
